import type { GraniteVec3 } from './glacialGraniteNoise'

export interface BandLimitedGraniteField {
  sdf(
    x: number,
    y: number,
    z: number,
...
export interface GraniteSurface {
  positions: Float64Array
  normals: Float64Array
  indices: Uint32Array
}

const CORNERS: ReadonlyArray<GraniteVec3> = [
...
export function extractGraniteSurface(options: {
  field: BandLimitedGraniteField
  seed: number
  cells: number
  regularization?: number
  ...
  const minimumWavelength = recipe.octaveBudget(cells).minimumWavelength
  const samples = cells + 1
  const step = 2 / cells
  const scalarField = new Float32Array(samples * samples * samples)
  for (let z = 0; z < samples; z += 1) {
    ...
    for (let y = 0; y < samples; y += 1) {
      ...
      for (let x = 0; x < samples; x += 1) {
        scalarField[x + rowOffset] = recipe.sdf(
          -1 + x * step,
          py,
          pz,
          seed,
  ...
  const sampleAt = (x: number, y: number, z: number) =>
    scalarField[x + samples * (y + samples * z)]!
  const vertexByCell = new Int32Array(cells * cells * cells).fill(-1)
  const positions: number[] = []
  const normals: number[] = []
  const crossingPoints = new Float64Array(36)
  const crossingNormals = new Float64Array(36)
  ...
  for (let z = 0; z < cells; z += 1) {
    for (let y = 0; y < cells; y += 1) {
      for (let x = 0; x < cells; x += 1) {
        ...
        if (count === 0) continue
        const centroid: GraniteVec3 = [cx / count, cy / count, cz / count]
        const solved = solveQef(
          crossingPoints,
          crossingNormals,
          count,
          centroid,
  ...
  const vertexAt = (x: number, y: number, z: number) =>
    vertexByCell[x + cells * (y + cells * z)]!
  const pushQuad = (
    outward: boolean,
    a: number,
    b: number,
