 ...
 */
/** One pooled shell. Returned by `spawn` so a caller can retune it mid-flight. */
export type BurstRecord = {
  ...
  radius: number;
  endRadius: number;
  /** Vertical scale, so a shell can read as a dome rather than a sphere. */
  squash: number;
};

export type BurstSpawnOptions = {
  radius?: number;
  endRadius?: number;
  life?: number;
  intensity?: number;
...
export class BurstSystem {
  ...
  readonly geometry: IcosahedronGeometry;

  private readonly pools = new Map<number, ObjectPool<BurstRecord>>();
  private readonly active: BurstRecord[] = [];
  ...
  private poolFor(mode: number): ObjectPool<BurstRecord> {
  ...
  private create(mode: number): BurstRecord {
   ...
   */
  spawn(mode: number, position: Vector3, options: BurstSpawnOptions = {}): BurstRecord {
    const {
      radius = 0.4,
      endRadius = 3,
      life = 0.9,
    ...
    const burst = this.poolFor(mode).acquire();
  ...
  update(dt: number): void {
    for (let i = this.active.length - 1; i >= 0; i--) {
      ...
      if (t >= 1) {
        ...
        this.poolFor(burst.mode).release(burst);
      }
    }
  }

  clear() {
    for (const burst of this.active) this.poolFor(burst.mode).release(burst);
