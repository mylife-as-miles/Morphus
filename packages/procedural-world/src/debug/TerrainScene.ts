...
import { Particles } from '../gpu/passes/Particles';
...
import { adaptAtmosphereConfig } from '../integration/config/AtmosphereConfigAdapter';
import { adaptLightingConfig } from '../integration/config/LightingConfigAdapter';
import { adaptMotionConfig } from '../integration/config/MotionConfigAdapter';
import { adaptPostConfig } from '../integration/config/PostConfigAdapter';
import { adaptTerrainConfig } from '../integration/config/TerrainConfigAdapter';
import { adaptVegetationConfig } from '../integration/config/VegetationConfigAdapter';
import { adaptWaterConfig } from '../integration/config/WaterConfigAdapter';

export type ProceduralWorldSceneBindings = {
  clouds: Clouds;
  ...
  sunSky: SunSky;
  terrainTiles: TerrainTiles | null;
  water: WaterSurface | null;
};

export async function buildTerrainScene(ctx: WorldContext): Promise<ProceduralWorldSceneBindings> {
  const { engine, params, seed } = ctx;
  const terrainConfig = adaptTerrainConfig(params.config);
  const vegetationConfig = adaptVegetationConfig(params.config);
  const lightingConfig = adaptLightingConfig(params.config);
  const atmosphereConfig = adaptAtmosphereConfig(params.config);
  const waterConfig = adaptWaterConfig(params.config);
  const motionConfig = adaptMotionConfig(params.config);
  const postConfig = adaptPostConfig(params.config);
  ...
  const sunSky = new SunSky(engine, bootTod, { ...lightingConfig, timeOfDay: bootTod });
  ...
  const scatter = await runScatter(engine.renderer, hf, seed, vegetationConfig);
  ...
  if (lightingConfig.giEnabled && !ablate.has('gi')) {
    gi = new ProbeGI(
      hf,
      sunSky.atmosphere,
    ...
    const giRef = gi;
    engine.onUpdate(() => giRef.tick(engine.renderer));
  }
  ...
  if (waterConfig.caustics && !ablate.has('caustics')) {
  ...
  if (!ablate.has('wind') && hf.noiseA) {
    ...
    windU.strength.value = motionConfig.windStrength;
    windU.vegetationResponse.value = vegetationConfig.windResponse;
    windU.dir.value.set(Math.cos(motionConfig.windDirection), Math.sin(motionConfig.windDirection));
  }

  ctx.progress(0.958, 'terrain: building tiles');
  const view = new URLSearchParams(window.location.search).get('view');
  let terrainTiles: TerrainTiles | null = null;
  ...
  if (view === 'split' && hf.preErosion) {
    ...
    const tiles = new TerrainTiles(hf, view, { gi: gi ?? undefined, canopyTex, wetMargins: waterConfig.wetMargins });
    terrainTiles = tiles;
    tiles.farShell.visible = terrainConfig.farShell;
  ...
  if (view !== 'split' && waterConfig.enabled && !ablate.has('water')) {
    ...
    engine.onUpdate(() => waterRef.update(engine.camera));
  }
  ...
  if (view !== 'scatter' && !ablate.has('veg')) {
    ...
    const forests = new Forests(
      ...
      gi,
      canopyTex,
      vegetationConfig.impostorRange,
    );
    ...
    if (!ablate.has('grass')) {
      const ring = new GroundRing(
        hf,
        canopyTex,
        new WorldSeed((seed.seed ^ (vegetationConfig.scatterSeedOffset >>> 0)) >>> 0),
        gi,
        vegetationConfig.grassDensity,
      );
  ...
  const clouds = new Clouds(sunSky.atmosphere, atmosphereConfig);
  ...
  const shadowRig = setupSunShadows(sunSky.sun, engine.camera, (wxz) =>
    clouds.shadowAt(wxz),
  {
    cascades: lightingConfig.cascadeCount,
    mapSize: lightingConfig.shadowMapResolution,
    maxFar: lightingConfig.maxShadowDistance,
  });
  ...
  if (view !== 'split' && motionConfig.particleTypes.length > 0 && !ablate.has('particles')) {
    const parts = new Particles(hf, canopyTex, gi, motionConfig);
    engine.scene.add(parts.mesh);
    engine.onUpdate((dt) => parts.update(engine.renderer, engine.camera, dt));
    engine.stats.counters['particles'] = parts.count;
  }

  // froxel volumetrics: canopy shafts + valley fog (?ablate=froxels, ?fog=N)
  let froxels: Froxels | null = null;
  if (atmosphereConfig.volumetrics && !ablate.has('froxels')) {
    froxels = new Froxels(hf, sunSky.atmosphere, canopyTex, clouds);
    froxels.fogK.value = atmosphereConfig.fogDensity;
    const fx = froxels;
  ...
  const post = new PostStack(engine, sunSky.atmosphere, bootTod, clouds, froxels, postConfig);
  engine.post = post;
  ...
  return { clouds, froxels, gi, heightfield: hf, post, scatter, sunSky, terrainTiles, water };
}
