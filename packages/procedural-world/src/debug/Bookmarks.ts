...
export function installBookmarks(
  ...
  const bookmarks: Bookmark[] = params.config.bookmarks.length > 0
    ? params.config.bookmarks.slice(0, 9).map((bookmark) => ({
        alt: bookmark.y - hf.heightAtCpu(bookmark.x, bookmark.z),
        name: bookmark.name,
        pitch: bookmark.pitch,
        tod: bookmark.timeOfDay,
        x: bookmark.x,
        yaw: bookmark.yaw,
        z: bookmark.z,
      }))
