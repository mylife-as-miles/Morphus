...
export function waterMaterial(
  ...
  const reflection = Fn((): NV3 => {
    ...
    const ssrSteps = lvl.reflectionQuality === 'low' ? 8 : lvl.reflectionQuality === 'high' ? 12 : 18;
    Loop(ssrSteps, ({ i }: { readonly i: NI }) => {
  ...
  const foam = clamp(shoreFoam.add(rapidFoam), 0, 1)
    .mul(foamPat)
    .mul(lvl.foamStrength)
    .clamp(0, 0.68) as NF;
