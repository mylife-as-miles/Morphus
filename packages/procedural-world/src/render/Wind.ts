...
export const windU = {
  ...
  /** authored vegetation response multiplier */
  vegetationResponse: runiform(1),
};
...
export function vegWindOffset(a: WindVertexArgs): NV3 {
  ...
  const flex = vd.y;
  const d = vec2(windU.dir as unknown as NV2);
  const s = (windU.strength as unknown as NF).mul(
    windU.vegetationResponse as unknown as NF,
  );
