...
import type { PostRuntimeConfig } from '../integration/config/PostConfigAdapter';
...
export class PostStack {
  ...
  constructor(
    ...
    config?: PostRuntimeConfig,
  ) {
    const { renderer, renderScene: scene, camera } = engine;
    ...
    if (config && !config.gtao) ablate.add('ao');
    if (config && !config.screenSpaceBounce) ablate.add('bounce');
    if (config && !config.taa) ablate.add('taa');
    if (config && !config.bloom) ablate.add('bloom');
    if (config && !config.autoExposure) this.lockExposure = true;
    ...
    const configuredDebug = config?.debugView ?? 'none';
    const configuredDebugNode = configuredDebug === 'ao' && aoFaded
      ? vec3(aoFaded)
      : configuredDebug === 'clouds' && cloudTex
        ? cloudTex.rgb
        : configuredDebug === 'velocity'
          ? vec3(velReproject(screenUV.mul(screenSize)), 0)
          : null;
    this.post.outputNode =
      skyVelDbgView !== null ? skyVelDbgView
      : configuredDebugNode !== null ? configuredDebugNode
