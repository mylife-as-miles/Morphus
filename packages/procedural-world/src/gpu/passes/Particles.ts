...
import type { MotionRuntimeConfig } from '../../integration/config/MotionConfigAdapter';
...
export class Particles {
  ...
  constructor(
    ...
    const particleCount = config?.particleCount ?? PARTICLE_COUNT;
    this.count = particleCount;
    const enabled = new Set(config?.particleTypes ?? ['pollen', 'snow', 'leaves']);
    const fallbackType = enabled.has('pollen') ? T_POLLEN : enabled.has('snow') ? T_SNOW : T_LEAF;
    const pollenType = enabled.has('pollen') ? T_POLLEN : fallbackType;
    const snowType = enabled.has('snow') ? T_SNOW : fallbackType;
    const leafType = enabled.has('leaves') ? T_LEAF : fallbackType;
    // (x, y, z, type) — type carries through until the particle re-rolls
    const pos = instancedArray(particleCount, 'vec4');
    // (phase, size01, age, ttl)
    const misc = instancedArray(particleCount, 'vec4');
    ...
    const rollType = (p: NV3, h: NF): NF => {
      ...
      return isSnow.select(
        float(snowType),
        leafRoll.select(float(leafType), float(pollenType)),
      );
    };

    this.stepK = Fn(() => {
      const i = instanceIndex;
      If(i.greaterThanEqual(particleCount), () => {
        Return();
      });
    ...
    const mesh = new InstancedMesh(geo, mat, particleCount);
