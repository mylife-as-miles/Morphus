...
import type { VegetationRuntimeConfig } from '../../integration/config/VegetationConfigAdapter';
...
export async function runScatter(
  ...
  const offset = config.scatterSeedOffset >>> 0;
  const sT = (seed.sub('scatter/trees') ^ offset) & 0x7fffffff;
  const sU = (seed.sub('scatter/understory') ^ offset) & 0x7fffffff;
  const sE = (seed.sub('scatter/extras') ^ offset) & 0x7fffffff;
  const species = new Set(config.enabledSpecies);
  const speciesWeights = [
    species.has('conifer') ? 1 : 0,
    species.has('pine') ? 1 : 0,
    species.has('beech') ? 1 : 0,
    species.has('birch') ? 1 : 0,
    species.has('oak') ? 1 : 0,
    species.has('willow') ? 1 : 0,
  ] as const;
  const anySpecies = speciesWeights.some((weight) => weight > 0) ? 1 : 0;
  ...
  const treeK = Fn(() => {
    ...
    const slopeFade = float(1).sub(
      smoothstep(0.5 * config.slopeLimit, 0.95 * config.slopeLimit, s.slope),
    );
    ...
    const accept = dens
      ...
      .mul(config.treeDensity * anySpecies);
    ...
    const w0 = byBiome(s.bioId, [0, 0.6, 0.58, 0.07, 0.05, 0.12]) // spruce
      .mul(m.mul(0.5).add(0.75)).mul(speciesWeights[0]);
    const w1 = byBiome(s.bioId, [0, 0.22, 0.27, 0.02, 0.15, 0]) // pine
      .mul(float(1.45).sub(m.mul(0.9))).mul(speciesWeights[1]);
    const w2 = byBiome(s.bioId, [0, 0, 0.02, 0.5, 0.42, 0.05]) // beech
      .mul(m.mul(0.9).add(0.55)).mul(speciesWeights[2]);
    const w3 = byBiome(s.bioId, [0, 0.03, 0.08, 0.16, 0.3, 0.55]) // birch
      .mul(m.mul(0.6).add(0.7)).mul(speciesWeights[3]);
    const w4 = byBiome(s.bioId, [0, 0, 0, 0.2, 0, 0]) // karst gnarl
      .mul(s.rockExp.mul(1.6).add(0.4)).mul(speciesWeights[4]);
    const w5 = byBiome(s.bioId, [0, 0.15, 0.05, 0.05, 0.08, 0.28])
      .mul(speciesWeights[5]); // snag
  ...
  const underK = Fn(() => {
    ...
    const slopeFade = float(1).sub(
      smoothstep(0.55 * config.slopeLimit, 0.9 * config.slopeLimit, s.slope),
    );
    const treelineFade = float(1).sub(
  ...
  const extraK = Fn(() => {
    ...
    const accept = dens.mul(slopeFade).mul(wSum.min(1)).mul(config.understoryDensity);
  ...
  const stoneK = Fn(() => {
    ...
    const accept = stoneBase.add(branchW).min(1).mul(config.grassDensity);
