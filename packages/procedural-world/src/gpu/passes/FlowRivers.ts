...
export async function runFlowRivers(
  ...
  const lakeDelta = opts.lakeBehavior === 'off' ? 1e9 : opts.lakeBehavior === 'connected' ? 1.35 : LAKE_DELTA;
  const riverThreshold = Math.max(0.1, opts.riverThreshold ?? 1);
  const moistureScale = Math.max(0, opts.moistureScale ?? 1);
  const { res, seed } = opts;
  const N = res * res;
  ...
  const traceK = Fn<void>(() => {
    ...
    Loop(STEPS, () => {
      ...
      If(W.element(i).sub(height.element(i)).greaterThan(lakeDelta), () => {
        Break();
      });
  ...
  const RIVER_T = (particles / N + 14) * riverThreshold;
  ...
  const WATER_T = (particles / N + 320) * riverThreshold;
  ...
  const carveK = guard(() => {
    ...
    const isLake = lakeD.greaterThan(lakeDelta);
    ...
    const lakeFade = smoothstep(lakeDelta * 0.7, 0.12, lakeD);
  ...
  if (moistureScale !== 1) {
    const moistureScaleK = guard(() => {
      const { i } = cellXY();
      moistA.element(i).assign(clamp(moistA.element(i).mul(moistureScale), 0, 1));
    })().compute(N);
    moistureScaleK.setName('moistureScale');
    await renderer.computeAsync(moistureScaleK);
  }
