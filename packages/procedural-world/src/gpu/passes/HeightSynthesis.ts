...
export async function runHeightSynthesis(
  ...
  res: number,
  mp: MacroParams,
  options: { heightAmplitude?: number; noiseScale?: number } = {},
  ...
  const kernel = Fn(() => {
    ...
    const wpos = vec2(float(x).add(0.5), float(y).add(0.5))
      ...
      .sub(0.5)
      .mul(WORLD_SIZE);
    const samplePos = wpos.mul(options.noiseScale ?? 1);
    const m = macroTerrain(samplePos, mp, 'full');
    height.element(i).assign(m.height.mul(options.heightAmplitude ?? 1));
