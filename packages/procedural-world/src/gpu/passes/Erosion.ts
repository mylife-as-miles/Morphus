...
export async function runErosion(
  ...
  const hydraulicScale = Math.max(0, opts.hydraulicScale ?? 1);
  const thermalScale = Math.max(0, opts.thermalScale ?? 1);
  const N = res * res;
  ...
  const makeHydra = (r: Roles): ComputeNode[] => {
    ...
    const waterK = guard(() => {
      ...
      const w2 = w1.mul(1 - EVAP * DT).add(RAIN * hydraulicScale * DT).toVar();
      If(isBorder(x, y), () => {
        w2.assign(0);
      });
    ...
    const erodeK = guard(() => {
      ...
      const cap = float(KC * hydraulicScale).mul(sinA).mul(speed).mul(shallowFade);
      const s0 = r.sSrc.element(i).toVar();
      const hard = hardness.element(i);
      const erodedAmt = min(
        MAX_ERODE_PER_ITER,
        max(0, cap.sub(s0))
          .mul(KS * hydraulicScale * DT)
  ...
  const thermalK = ((): ComputeNode => {
    const k = guard(() => {
      ...
      const rate0 = float(1).sub(hard0).pow(1.5).mul(THERMAL_RATE * thermalScale * DT * texel);
      let net: NF = float(0);
      ...
      for (const [ox, oy] of offs) {
        ...
        const rateN = float(1).sub(hardN).pow(1.5).mul(THERMAL_RATE * thermalScale * DT * texel);
