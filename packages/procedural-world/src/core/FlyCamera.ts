...
export class FlyCamera {
  ...
  constructor(camera: PerspectiveCamera, dom: HTMLElement) {
    ...
    window.addEventListener('keyup', (e) => this.keys.delete(e.code), listenerOptions);
    window.addEventListener('blur', () => this.keys.clear(), listenerOptions);
  ...
  configureMovement(config: { flySpeed: number; sprintMultiplier: number; walkSpeed: number }): void {
  ...
  private updateWalk(dt: number): void {
    ...
    if (MOVE.lengthSq() > 0) {
      MOVE.normalize();
      target = this.walkSpeed * (sprinting ? this.sprintMultiplier : 1);
    ...
    const speedK = Math.min(speedH / this.walkSpeed, this.sprintMultiplier);
    ...
    if (this.grounded && speedH > 0.3) {
      const rate = STRIDE_RATE * this.walkSpeed * (0.55 + 0.45 * Math.min(speedK, 2));
    ...
    // sprint FOV kick
    const fovTarget = sprinting && this.grounded && speedH > this.walkSpeed * 1.15 ? SPRINT_FOV_ADD : 0;
