...
export function parseParams(search: string = window.location.search): LaasParams {
  ...
  const config = createDefaultProceduralWorldConfig(Math.floor(num(q.get('seed'), 1)) >>> 0);
  config.preset = preset;
  config.timeOfDay = Math.min(24, Math.max(0, num(q.get('T'), 11)));
  config.motion.freezeSimulation = q.get('freeze') === '1';
  return {
    config: resolveProceduralWorldPreset(config).config,
