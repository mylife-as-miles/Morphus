...
export class GroundRing {
  ...
  init(beechAtlas: DataTexture | null): void {
    ...
    const grassK = Fn(() => {
      ...
      If(cellHash(wc, salt ^ 0x77a1).greaterThanEqual(dens.mul(this.density).mul(edge).mul(thin)), () => {
        Return();
      });
    ...
    const debrisK = Fn(() => {
      ...
      If(cellHash(wc, salt ^ 0x132f).greaterThanEqual(dens.mul(this.density).mul(edge)), () => {
        Return();
      });
    ...
    const farK = Fn(() => {
      ...
      If(
        cellHash(wc, salt ^ 0x55aa).greaterThanEqual(
          dens.mul(this.density).mul(fadeIn).mul(edge).mul(0.55),
        ),
        () => {
          Return();
        },
