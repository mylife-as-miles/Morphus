...
export class Heightfield {
  ...
  static async generate(
    ...
    const terrain = adaptTerrainConfig(params.config);
    const baseCfg = qualityConfig(params.preset);
    const cfg: QualityConfig = {
      ...baseCfg,
      heightRes: terrain.heightfieldResolution,
      simRes: terrain.simulationResolution,
      erosionIters: Math.round(baseCfg.erosionIters * Math.max(terrain.hydraulicErosion, terrain.thermalErosion)),
    };
    const mp = makeMacroParams(seed);

    progress(0.04, `terrain: synthesizing ${cfg.heightRes}² heightfield`);
    const synthesisOptions = { heightAmplitude: terrain.heightAmplitude, noiseScale: terrain.noiseScale };
    const synth = await runHeightSynthesis(renderer, cfg.heightRes, mp, synthesisOptions);
    ...
    const synthSim = await runHeightSynthesis(renderer, cfg.simRes, mp, synthesisOptions);
    ...
    const erosion = await runErosion(renderer, synthSim.height, synthSim.hardness, {
      ...
      hydraulicScale: terrain.hydraulicErosion,
      thermalScale: terrain.thermalErosion,
    ...
    hf.flow = await runFlowRivers(renderer, erosion.eroded, erosion.water, {
      ...
      lakeBehavior: terrain.lakeBehavior,
      moistureScale: terrain.moisture,
      riverThreshold: terrain.riverThreshold,
