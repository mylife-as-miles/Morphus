...
import type { WaterRuntimeConfig } from '../integration/config/WaterConfigAdapter';
...
export class WaterSurface {
  ...
  private readonly foamStrength = runiform(1);
  ...
  constructor(
    ...
    gi: ProbeGI | null,
    config: WaterRuntimeConfig,
  ) {
    this.foamStrength.value = config.foam ? 1 : 0;
    const geo = gridGeometry();
    const activeCells = LEVEL_CELL.filter(
      (cell) => cell * CELLS * 0.5 <= config.clipmapDistance * 1.6,
    );
    for (const cell of activeCells.length > 0 ? activeCells : LEVEL_CELL.slice(0, 1)) {
      ...
      const mat = waterMaterial(hf, atm, canopyTex, gi, {
        ...
        cell,
        foamStrength: this.foamStrength,
        reflectionQuality: config.reflectionQuality,
  ...
  setFoamEnabled(enabled: boolean): void {
    this.foamStrength.value = enabled ? 1 : 0;
  }
