...
import type { AtmosphereRuntimeConfig } from '../integration/config/AtmosphereConfigAdapter';
...
export class Clouds {
  ...
  constructor(atmosphere: Atmosphere, config?: AtmosphereRuntimeConfig) {
    this.atmosphere = atmosphere;
    this.coverage.value = config?.cloudCoverage ?? this.coverage.value;
    this.speed.value = config?.cloudSpeed ?? 1;
  ...
  async refreshShadow(renderer: Renderer): Promise<void> {
    this.uDriftBase.value
      ...
      .multiplyScalar(this.timeAcc * this.DRIFT_V * this.speed.value);
  ...
  private driftAt(t: NF): NV2 {
    return vec2(windU.dir as unknown as NV2).mul(t.mul(this.DRIFT_V).mul(this.speed));
  }
